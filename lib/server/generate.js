...
async function execute() {
  ...
  if (fs.existsSync(`${CWD}/versioned_sidebars`)) {
    ...
    let latestSidebar = undefined;

    versions.reverse().forEach((e, i) => {
      var file = versionedSidebars.find(f => f.name.indexOf(e) > -1);
      
      if (file){
        latestSidebar = file;
      }
      
      const content = fs.readFileSync(latestSidebar.filepath);
